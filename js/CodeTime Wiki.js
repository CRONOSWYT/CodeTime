document.addEventListener("DOMContentLoaded", () => {
  ...
  function canViewAdmin() {
    return (
      currentProfile &&
      (currentProfile.role_id === ROLES.ADMIN ||
        currentProfile.role_id === ROLES.OWNER)
    );
  }
  ...
  async function getUser() {
    // Always check session from Supabase; do not rely solely on localStorage for authoritative profile
    const { data } = await supabase.auth.getSession();
    user = data?.session?.user || null;

    // Update localStorage user/session for continuity
    try {
      if (user) {
      ...
      console.warn("Error manejando localStorage user/session:", e);
    }
    ...
    if (user) {
      // Always fetch the authoritative profile from DB
      try {
        ...
        if (error) {
          console.warn("Error leyendo perfil desde DB:", error.message);
          currentProfile = null;
        } else {
          currentProfile = perfilDb || null;
          // persist a lightweight copy for faster access in other pages (not authoritative)
          try {
            if (currentProfile)
              localStorage.setItem("perfil", JSON.stringify(currentProfile));
          } catch (e) {}
        }
      } catch (err) {
      ...
      if (isUsernameMissing(currentProfile)) {
        ...
        // Show modal to force username completion
        showProfileModal();
      } else {
      ...
      if (canViewAdmin()) {
        // Evitar duplicar el botón Admin si ya existe
        if (!document.getElementById("boton_admin")) {
          const adminBtn = document.createElement("button");
          adminBtn.id = "boton_admin";
  ...
  function showProfileModal() {
    ...
    document.getElementById("cerrar_modal_perfil").style.display =
      usernameRequired ? "none" : "flex";
  }
  ...
  async function saveProfile() {
    ...
    let error = null;
    const oldValues = currentProfile
    ...
    if (currentProfile) {
      ...
      if (!error) {
        await logAudit(
          ...
          "perfil",
          user.id,
          `Username actualizado de ${currentProfile.username} a ${usernameValue}`,
          oldValues,
          newValues,
        );
      }
    ...
    // Guardar perfil en localStorage para que otras páginas lo detecten
    try {
      localStorage.setItem("perfil", JSON.stringify(currentProfile));
    } catch (e) {
...
`;
  ...
  function openReportModal(entityType, targetId, targetName, targetOwnerId) {
    ...
    document.getElementById("titulo_modal_reporte").textContent =
      `Reportar ${entityType}`;
    document.getElementById("objetivo_reporte").value =
  ...
  submitReportBtn.onclick = async () => {
    const description = document
      .getElementById("descripcion_reporte")
      .value.trim();
    ...
    await logAudit(
      ...
      "reports",
      data[0].id,
      `Reporte creado: ${reportTarget.entityType} - ${reportTarget.targetName}`,
      null,
      {
        entity_type: reportTarget.entityType,
  ...
  window.onclick = (event) => {
    ...
    if (
      event.target === document.getElementById("modal_perfil") &&
      !usernameRequired
    ) {
      hideProfileModal();
    }
