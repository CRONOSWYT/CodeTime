document.addEventListener("DOMContentLoaded", () => {
  ...
  const clearSearchCards = document.getElementById("boton_limpiar_busqueda_tarjetas_admin");
  const deleteSelectedCards = document.getElementById("boton_eliminar_tarjetas_admin");
  ...
  const clearSearchLanguages = document.getElementById("boton_limpiar_busqueda_lenguajes_admin");
  const deleteSelectedLanguages = document.getElementById("boton_eliminar_lenguajes_admin");
  ...
  const clearSearchUsers = document.getElementById("boton_limpiar_busqueda_usuarios_admin");
  const deleteSelectedUsers = document.getElementById("boton_eliminar_usuarios_admin");
  ...
  const clearSearchReports = document.getElementById("boton_limpiar_busqueda_reportes_admin");
  const deleteSelectedReports = document.getElementById("boton_eliminar_reportes_admin");
  ...
  const clearSearchLogs = document.getElementById("boton_limpiar_busqueda_logs_admin");
  ...
  const botonAplicarFiltroFechas = document.getElementById("boton_aplicar_filtro_fechas");
  const botonLimpiarFiltroFechas = document.getElementById("boton_limpiar_filtro_fechas");
  
  const adminModal = document.getElementById("modal_admin");
  const closeAdminModal = document.getElementById("cerrar_modal_admin");
  const saveAdminModalBtn = document.getElementById("boton_guardar_modal_admin");
  const cancelAdminModalBtn = document.getElementById("boton_cancelar_modal_admin");
  ...
  const cerrarModalDetallesLog = document.getElementById("cerrar_modal_detalles_log");
  const botonCerrarDetallesLog = document.getElementById("boton_cerrar_detalles_log");
  const cuerpoModalDetallesLog = document.getElementById("cuerpo_modal_detalles_log");
  ...
  async function getUser() {
    ...
    if (!perfil || (perfil.role_id !== ROLES.ADMIN && perfil.role_id !== ROLES.OWNER)) {
      Swal.fire({
        icon: "error",
  ...
  async function loadUsers() {
    ...
    usersMap = new Map(allUsers.map((userItem) => [userItem.id, userItem.username]));
    usersRoleMap = new Map(allUsers.map((userItem) => [userItem.id, userItem.role_id]));
  ...
  async function loadAuditLogs() {
    ...
    allAuditLogs = data || [];
    const userIds = [...new Set(allAuditLogs.map((log) => log.user_id).filter(Boolean))];
    ...
    if (userIds.length > 0) {
      const { data: profiles } = await supabase
        ...
        .select("id, username")
        .in("id", userIds);
      auditLogUserNames = new Map((profiles || []).map((p) => [p.id, p.username]));
    }
  ...
  function renderLogs(logs, userNames = auditLogUserNames) {
    ...
    logs.forEach((logItem) => {
      ...
      el.innerHTML = `
        ...
        <td>
          <button class="btn btn_detalles_log" data-log-id="${logItem.id}" style="padding: 0.5rem 1rem; font-size: 0.85rem;">
      ...
      el.querySelector(".btn_detalles_log").onclick = () => {
  ...
  searchLogs.oninput = () => {
    ...
    const filtered = allAuditLogs.filter(
      (logItem) =>
        (String(logItem.action_type || "")).toLowerCase().includes(query) ||
        (String(logItem.entity_type || "")).toLowerCase().includes(query) ||
        (String(logItem.entity_id || "")).toLowerCase().includes(query) ||
        (String(logItem.description || "")).toLowerCase().includes(query) ||
  ...
  botonAplicarFiltroFechas.onclick = () => {
    ...
    if (!fechaInicio || !fechaFin) {
      Swal.fire({ icon: "warning", title: "Error", text: "Selecciona ambas fechas" });
      return;
    }
    ...
    const filtered = allAuditLogs.filter((logItem) => {
      ...
      return logDate >= inicioDate && logDate <= finDate;
    });
  ...
  deleteSelectedCards.onclick = async () => {
    ...
    if (error) {
      ...
      await logAudit('DELETE', 'cards', selected.join(','), `${selected.length} tarjeta(s) eliminadas`);
      loadCards();
      Swal.fire({ icon: "success", title: "Éxito", text: "Tarjetas eliminadas" });
    }
  };
  ...
  deleteSelectedLanguages.onclick = async () => {
    ...
    if (error) {
      ...
      await logAudit('DELETE', 'languages', selected.join(','), `${selected.length} lenguaje(s) eliminados`);
      loadLanguages();
      Swal.fire({ icon: "success", title: "Éxito", text: "Lenguajes eliminados" });
    }
  };
  ...
  deleteSelectedUsers.onclick = async () => {
    ...
    if (error) {
      ...
      await logAudit('DELETE', 'perfiles', selected.join(','), `${selected.length} usuario(s) eliminados`);
      loadUsers();
      Swal.fire({ icon: "success", title: "Éxito", text: "Usuarios eliminados" });
    }
  };
  ...
  deleteSelectedReports.onclick = async () => {
    ...
    if (error) {
      ...
      await logAudit('DELETE', 'reports', selected.join(','), `${selected.length} reporte(s) eliminados`);
      loadReports();
      Swal.fire({ icon: "success", title: "Éxito", text: "Reportes eliminados" });
    }
  };
  ...
  function openAdminModal(type, item) {
      ...
      case "card":
        adminModalBody.innerHTML = `
          ...
          <div class="grupo_formulario">
            ...
            <textarea id="campo_contenido_admin">${item.contenido || ""}</textarea>
            <div class="previsualizacion_editor">
              <div class="previsualizacion_encabezado">Vista previa</div>
              <div class="previsualizacion_contenido" id="preview_contenido_admin"></div>
            </div>
          </div>
          <div class="grupo_formulario">
    ...
    saveAdminModalBtn.style.display = currentAdminType === "report" && !isOwner() ? "none" : "inline-block";
    connectAdminEditor();
  }

  function connectAdminEditor() {
    const contentTextarea = adminModalBody.querySelector("#campo_contenido_admin");
    const descriptionTextarea = adminModalBody.querySelector("#campo_descripcion_admin2");
    ...
    toolbarButtons.forEach((button) => {
      button.onclick = (e) => {
        ...
        const targetTextarea = activeTextarea || contentTextarea || descriptionTextarea;
  ...
  function updateAdminPreview() {
    const outputContent = adminModalBody.querySelector("#preview_contenido_admin");
    const outputDescription = adminModalBody.querySelector("#preview_descripcion_admin");

    if (outputContent) {
      outputContent.innerHTML = marked.parse(adminModalBody.querySelector("#campo_contenido_admin").value || "");
    }
    if (outputDescription) {
      outputDescription.innerHTML = marked.parse(adminModalBody.querySelector("#campo_descripcion_admin2").value || "");
    }
  }
  ...
  async function saveAdminModal() {
    ...
    if (currentAdminType === "card") {
      ...
      const descripcion = document.getElementById("campo_descripcion_admin").value.trim();
      const contenido = document.getElementById("campo_contenido_admin").value.trim();
      const language_id = document.getElementById("campo_id_lenguaje_admin").value;

      if (!titulo || !descripcion) {
        Swal.fire({ icon: "warning", title: "Error", text: "Todos los campos son obligatorios" });
        return;
      }
      ...
      if (error) {
        return Swal.fire({ icon: "error", title: "Error", text: error.message });
      }
      await logAudit('UPDATE', 'cards', currentAdminItem.id, `Tarjeta actualizada: ${titulo}`, oldValues, newValues);
      loadCards();
    }
    ...
    if (currentAdminType === "language") {
      ...
      const description = document.getElementById("campo_descripcion_admin2").value.trim();
      ...
      await logAudit('UPDATE', 'languages', currentAdminItem.id, `Lenguaje actualizado: ${name}`, oldValues, newValues);
      loadLanguages();
    }

    if (currentAdminType === "user") {
      const username = document.getElementById("campo_usuario_admin").value.trim();
      const role_id = parseInt(document.getElementById("campo_id_rol_admin").value, 10);

      if (!username) {
        Swal.fire({ icon: "warning", title: "Error", text: "El nombre de usuario es obligatorio" });
        return;
      }
      ...
      await logAudit('UPDATE', 'perfiles', currentAdminItem.id, `Usuario actualizado: ${username}, Rol: ${role_id}`, oldValues, newValues);
      loadUsers();
    }

    if (currentAdminType === "report") {
      const status = document.getElementById("campo_estado_reporte_admin").value;
      ...
      await logAudit('UPDATE', 'reports', currentAdminItem.id, `Estado del reporte actualizado a: ${status}`, oldValues, newValues);
      loadReports();
    }
